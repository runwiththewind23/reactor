...
TcpServer::TcpServer(const std::string &ip, const uint16_t port, int threadnum)
  ...
  acceptor_.setnewconnnectioncb(
      std::bind(&TcpServer::newconnection, this, std::placeholders::_1));

  //创建从事件循环
  for (int ii = 0; ii < threadnum_; ++ii) {
    subloops_.emplace_back(
        new EventLoop(false, 5, 10));  //创建从事件循环，放入subloops容器中
    subloops_[ii]->setepolltimeoutcallback(
        std::bind(&TcpServer::epolltimeout, this,
                  std::placeholders::_1));  //设置timeout超时的回调
    subloops_[ii]->settimercallback(
        std::bind(&TcpServer::removeconn, this, std::placeholders::_1));
    threadpool_.addtask(std::bind(
        &EventLoop::run, subloops_[ii].get()));  //在线程池中加入从事件循环
    sleep(1);
  }
}
TcpServer::~TcpServer() {}
...
void TcpServer::newconnection(std::unique_ptr<Socket> clientsock) {
  ...
  subloops_[conn->fd() % threadnum_]->newconnection(
      conn);  //把conn存放到EventLoop的map容器中
...
void TcpServer::sendcomplete(
    spConnection conn) {  //数据发送完成后，在connection中回调
  // printf("send complete.\n");
  if (sendcompletecb_) sendcompletecb_(conn);
}

void TcpServer::epolltimeout(
    EventLoop *loop) {  // epoll_wait超时，在Eventloop类中回调
