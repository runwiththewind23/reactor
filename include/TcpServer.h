...
class TcpServer {
 private:
  ...
  std::map<int, spConnection>
      conns_;  //一个TCPServer有多个Connection对象，存放在map容器中
 ...
 public:
  ...
  void stop();
  void newconnection(
      std::unique_ptr<Socket> clientsock);  //处理新客户端连接请求
  ...
  void removeconn(
      int fd);  //删除conns_中的connection对象，在eventloop::handletimer中回调
};
}  // namespace reactor