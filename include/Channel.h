 ...
 * @LastEditTime: 2024-07-31 16:36:54
 * @FilePath: /sxd/reactor/reactor/39/include/Channel.h
...
class Channel {
 private:
  ...
  bool inepoll_ =
      false;  // channel是否已添加到epoll树上，如果未添加，调用时用EPOLL_CTL_ADD，否则用EPOLL_CTL_MOD
  uint32_t events_ =
      0;  // fd_需要监视的事件，listenfd和clientfd还可能需要监视EPOLLOUT
