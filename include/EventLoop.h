 ...
 * @LastEditTime: 2024-07-31 16:38:05
 * @FilePath: /sxd/reactor/reactor/39/include/EventLoop.h
...
class EventLoop {
 private:
  ...
  std::function<void(EventLoop *)>
      epolltimeoutcallback_;  // epoll_wait()超时的回调函数
  pid_t threadid_;            //事件循环所在线程的id
  std::queue<std::function<void()>>
      taskqueue_;  //事件循环线程被eventfd唤醒后执行的任务队列
  ...
  std::function<void(int)>
      timercallback_;  //删除TCPserver中的connection对象，将被设置为TCPserver::removeconn()的回调
 ...
 public:
  ...
  void updatechannel(
      Channel *
          ch);  //把channel添加/更新到红黑树中，channel中有fd，也有需要监视的事件
  void setepolltimeoutcallback(
      std::function<void(EventLoop *)> fn);  //设事件循环的回调
  ...
  void settimercallback(
      std::function<void(int)> fn);  //被设置为TCPserver::removeconn()的回调
};
}  // namespace reactor