 ...
 * @LastEditTime: 2024-07-31 16:37:15
 * @FilePath: /sxd/reactor/reactor/39/include/Connection.h
...
class Connection : public std::enable_shared_from_this<Connection> {
 private:
  ...
  std::unique_ptr<Socket>
      clientsock_;  //服务端用于监听的socket，在构造函数中创建
  std::unique_ptr<Channel>
      clientchannel_;  // Acceptor对应的channel，在构造函数中创建(160字节)
  ...
  std::function<void(spConnection)>
      closecallback_;  //关闭fd_的回调函数，将回调TCp::closeconnection
  std::function<void(spConnection)> errorcallback_;  // fd_发生错误的回调函数
  std::function<void(spConnection, std::string &)>
      onmessagecallback_;  //处理报文的回调函数
  std::function<void(spConnection)>
      sendcompletecallback_;  //发送数据完成后的回调函数
  Timestamp
      lasttime_;  //时间戳，创建Connection对象为当前时间，没收到一个报文，更新为当前时间
 ...
 public:
  ...
  std::string
  ip();  //如果是listenfd存放服务端监听的ip，如果是客户端，存放对端的ip
  uint16_t
  port();  //果是listenfd存放服务端监听的port，如果是客户端，存放对端的port
  ...
  void send(const char *data,
            size_t size);  //发送数据,不管在任何线程中，都调用此函数
  void sendinloop(
      std::shared_ptr<std::string>
          data);  //发送数据，如果当前是IO线程，直接调用，如果是工作线程，将此函数传递给IO线程
  void setsendcompletecallback(
      std::function<void(spConnection)> fn);  //设置数据发送完成后的回调函数
